# tb_ki_ata_pio_write: the game's 256-store ATA PIO write block through the
...
$buildDir = Join-Path $simSource "ata_pio_write_build"
...
try {
    ...
    & $vsim -c -t 1ps -L altera_mf -L altera_mf_ver -L lpm -L sgate -L altera -L cyclonev `
        work.tb_ki_ata_pio_write `
        -do "set NumericStdNoWarnings 1; set StdArithNoWarnings 1; run -all; quit -f"
    if ($LASTEXITCODE -ne 0) { throw "ATA PIO write simulation failed" }
}
finally {
    Pop-Location
}
