...
$build = Join-Path $root 'sim/instrcache_build'
...
try {
    ...
    foreach ($f in @('functions.vhd', 'export.vhd', 'dpram.vhd', 'cpu_instrcache.vhd')) {
        & $vcom -quiet -2008 -work work (Join-Path $root "rtl/cpu/$f")
        if ($LASTEXITCODE -ne 0) { throw "vcom failed for $f" }
    }

    & $vlog -sv -quiet -work work (Join-Path $root 'sim/tb_ki_instrcache.sv')
    if ($LASTEXITCODE -ne 0) { throw 'vlog failed for the testbench' }

    $out = & $vsim -c -quiet -t 1ps -L mem -do 'run -all; quit -f' work.tb_ki_instrcache 2>&1
    $out | ForEach-Object { Write-Output $_ }
    if ($out -match '\*\* (Fatal|Error)') { throw 'instruction cache test failed' }
} finally {
    Pop-Location
}
