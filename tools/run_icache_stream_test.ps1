...
$buildDir = Join-Path $simSource "icache_stream_build"
...
try {
    ...
    foreach ($file in @("RamMLAB.vhd", "SyncFifoFallThroughMLAB.vhd", "dpram.vhd")) {
        & $vcom -quiet -2008 -work mem (Join-Path $cpuSource $file)
    ...
    foreach ($file in @(
        ...
        & $vcom -quiet -2008 -work work (Join-Path $cpuSource $file)
        if ($LASTEXITCODE -ne 0) { throw "vcom failed for $file (work)" }
    }
    & $vcom -quiet -2008 -work work (Join-Path $simSource "ki_cpu_wrapper.vhd")
